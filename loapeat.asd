(in-package :cl-user)
(defpackage loapeat-asd
  (:use :cl :asdf))
(in-package :loapeat-asd)
                ...
                :components ((:file "config")
			     ...
			     (:module "resources"
			      :components ((:file "resource")
					   ...
					   (:file "learner-slide-selection" :depends-on ("template" "slide"))
					   ...
					   (:module "api"
					    :components ((:file "api")
							 (:file "slide" :depends-on ("api"))
							 (:file "user" :depends-on ("api"))
							 (:file "semantic" :depends-on ("api"))
							 (:file "presentation" :depends-on ("api"))
							 (:file "learning-domain" :depends-on ("api"))
							 (:file "lesson" :depends-on ("api"))
							 ))
					   )))))
  :description "Meta-Learning Support System by using presentation slides"
  :in-order-to ((test-op (load-op cl-loapeat-test))))
